...
public class BaseRefreshLayout extends ViewGroup implements NestedScrollingParent, NestedScrollingChild, IPull {
    ...
    public BaseRefreshLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray array = context.obtainStyledAttributes(attrs, R.styleable.RefreshLayoutEnable);
        mPullDownEnable = array.getBoolean(R.styleable.RefreshLayoutEnable_pullDownEnable, true);
        mPullUpEnable = array.getBoolean(R.styleable.RefreshLayoutEnable_pullUpEnable, true);
        array.recycle();
    }
    ...
    private void updateLayout(float overScroll) {
        if (mScrollY != overScroll) {
            ...
            if (mScrollY >= 0) {
                ...
                mLoadFooter.getTargetView(this).requestLayout();
            }
        }
    }
