...
public class PinyinUtils {
    ...
    /**
     * First and last Chinese character with known PinyinUtils according to zh collation
     */
    ...
    private static class Token {
        ...
        String source;
        /**
         * Translated string of source. For Han, target is corresponding PinyinUtils. Otherwise target is
         * original string in source.
         */
        String target;
    }

    private PinyinUtils(boolean hasChinaCollator) {
        mHasChinaCollator = hasChinaCollator;
    }

    private static PinyinUtils getInstance() {
        synchronized (PinyinUtils.class) {
            ...
            for (Locale aLocale : locale) {
                if (aLocale.equals(Locale.CHINESE) || aLocale.equals(Locale.CHINA)) {
                    // Do self validation just once.
                    instance = new PinyinUtils(true);
                    return instance;
                }
            }
            instance = new PinyinUtils(false);
            return instance;
        }
    }
    ...
    public ArrayList<Token> get(final String input) {
        ...
        // b. Get the PinyinUtils of current charater.
     ...
     *
     * @param input       字符串
     * @param isLowerCase 返回字符是否转换成小写
     * @return 汉字返回拼音，字母原样返回，默认为大写
     */
    public static String getPinYin(String input, boolean isLowerCase) {
        List<Token> tokens = PinyinUtils.getInstance().get(input);
     ...
     *
     * @param input 字符串
     * @return 汉字返回拼音，字母原样返回，默认为大写,如果首字符不是字母则返回 defaultValue
     */
    public static String getPinYin(String input, String defaultValue) {
        return getPinYin(input, false, defaultValue);
    }
     ...
     * @param input        字符串
     * @param isLowerCase  返回字符是否转换成小写
     * @param defaultValue 默认字符，首字符不是字母的时候返回
     * @return 汉字返回拼音，字母原样返回，默认为大写,如果首字符不是字母则返回 defaultValue
     */
    public static String getPinYin(String input, boolean isLowerCase, String defaultValue) {
        String result = getPinYin(input, isLowerCase);
        if (defaultValue != null && !result.substring(0, 1).toUpperCase().matches("[A-Z]")) {
            return defaultValue;
        }
        return result;
    }
